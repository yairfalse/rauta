...
async fn test_p0_gatewayclass_acceptance() {
    ...
    let gc_name = format!("rauta-test-{}", &ctx.namespace[12..]);
...
"#,
    ...
    println!("  Step 5: Waiting for HTTPRoute reconciliation (5s)");
    tokio::time::sleep(Duration::from_secs(5)).await;

    // Step 6: Test traffic through RAUTA
    println!("  Step 6: Testing traffic through RAUTA (localhost:8080)");
    ...
    // In kind with port mapping 80->8080, RAUTA is at localhost:8080
    let response = client.get("http://localhost:8080/test/hello").send().await;
...
"#,
    ...
    // Step 3: Scale to 3
    println!("  Step 3: Scaling to 3 replicas");
    ctx.scale("deployment/scale-backend", 3)
        .await
        .expect("Should scale");

    // Wait for all replicas
    println!("  Step 4: Waiting for 3 replicas to be ready");
    ctx.wait_ready("deployment/scale-backend")
        .await
        .expect("Scaled backend should be ready");

    // Give EndpointSlice watcher time to update
    tokio::time::sleep(Duration::from_secs(5)).await;

    // Step 5: Verify traffic still works
    println!("  Step 5: Verifying traffic works after scale");
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(5))
        .build()
        .expect("Should build HTTP client");

    let response = client.get("http://localhost:8080/scale/test").send().await;
